    ...
    public class PlayListEditorWindow : EditorWindow {
        ...
        void PlayListSelected(ReorderableList list) {
            ...
            playListEntryView = new ReorderableList(selectedPlayList.entries, typeof(PlayListEntry), true, true, true, true) {
                ...
                elementHeight = (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing) * 9,
                showDefaultBackground = false,
            };
        }
        ...
        void DrawPlayListEntry(Rect rect, int index, bool isActive, bool isFocused) {
            ...
            var startRect = rect;
            startRect.yMin = urlQuestRect.yMax + EditorGUIUtility.standardVerticalSpacing;
            startRect.height = EditorGUIUtility.singleLineHeight;
            using (var changed = new EditorGUI.ChangeCheckScope()) {
                startRect = EditorGUI.PrefixLabel(startRect, Utils.GetTempContent("Start Time"));
                var newStart = EditorGUI.IntField(startRect, entry.start);
                if (changed.changed) {
            ...
            var endRect = rect;
            endRect.yMin = startRect.yMax + EditorGUIUtility.standardVerticalSpacing;
            endRect.height = EditorGUIUtility.singleLineHeight;
            using (var changed = new EditorGUI.ChangeCheckScope()) {
                endRect = EditorGUI.PrefixLabel(endRect, Utils.GetTempContent("End Time"));
                var newEnd = EditorGUI.IntField(endRect, entry.end);
                if (changed.changed) {
                    entry.end = newEnd;
                    selectedPlayList.entries[index] = entry;
                    isDirty = true;
            ...
            var flipRect = rect;
            flipRect.yMin = endRect.yMax + EditorGUIUtility.standardVerticalSpacing;
            flipRect.height = EditorGUIUtility.singleLineHeight;
            using (var changed = new EditorGUI.ChangeCheckScope()) {
                flipRect = EditorGUI.PrefixLabel(flipRect, Utils.GetTempContent("Flip Video"));
                var newFlip = EditorGUI.Toggle(flipRect, entry.flip);
                if (changed.changed) {
                    entry.flip = newFlip;
                    selectedPlayList.entries[index] = entry;
                    isDirty = true;
            ...
            var idRect = rect;
            idRect.yMin = flipRect.yMax + EditorGUIUtility.standardVerticalSpacing;
            idRect.height = EditorGUIUtility.singleLineHeight;
            using (var changed = new EditorGUI.ChangeCheckScope()) {
                idRect = EditorGUI.PrefixLabel(idRect, Utils.GetTempContent("ID"));
                var newId = EditorGUI.IntField(idRect, entry.id);
                if (changed.changed) {
                    entry.id = newId;
                    selectedPlayList.entries[index] = entry;
                    isDirty = true;
            ...
            var spellRect = rect;
            spellRect.yMin = idRect.yMax + EditorGUIUtility.standardVerticalSpacing;
            spellRect.height = EditorGUIUtility.singleLineHeight;
            using (var changed = new EditorGUI.ChangeCheckScope()) {
                spellRect = EditorGUI.PrefixLabel(spellRect, Utils.GetTempContent("Title Spell"));
                var newSpell = EditorGUI.TextField(spellRect, entry.titleSpell);
                if (changed.changed) {
                    entry.titleSpell = newSpell;
                    selectedPlayList.entries[index] = entry;
                    isDirty = true;
                }
            }
        ...
        void AppendPlaylist(UnityObject frontendHandler) {
            using (var serializedObject = new SerializedObject(frontendHandler)) {
                ...
                var playListEntryTitleSpellsProperty = serializedObject.FindProperty("playListEntryTitleSpells");
                var playListEntryIdsProperty = serializedObject.FindProperty("playListEntryIds");
                var playListEntryStartsProperty = serializedObject.FindProperty("playListEntryStarts");
                var playListEntryEndsProperty = serializedObject.FindProperty("playListEntryEnds");
                var playListEntryVolumesProperty = serializedObject.FindProperty("playListEntryVolumes");
                var playListEntryFlipsProperty = serializedObject.FindProperty("playListEntryFlips");
                ...
                for (int i = 0; i < playListCount; i++) {
                    ...
                    for (int j = 0; j < urlCount; j++)
                        playList.entries.Add(new PlayListEntry {
                            id = playListEntryIdsProperty.GetArrayElementAtIndex(urlOffset + j).intValue,
                            title = playListEntryTitlesProperty.GetArrayElementAtIndex(urlOffset + j).stringValue,
                            titleSpell = playListEntryTitleSpellsProperty.GetArrayElementAtIndex(urlOffset + j).stringValue,
                            ...
                            start = playListEntryStartsProperty.GetArrayElementAtIndex(urlOffset + j).intValue,
                            end = playListEntryEndsProperty.GetArrayElementAtIndex(urlOffset + j).intValue,
                            volume = playListEntryVolumesProperty.GetArrayElementAtIndex(urlOffset + j).floatValue,
                            flip = playListEntryFlipsProperty.GetArrayElementAtIndex(urlOffset + j).boolValue,
        ...
        void SerializePlayList() {
            ...
            using (var serializedObject = new SerializedObject(frontendHandler)) {
                ...
                playListEntryTitleSpellsProperty.arraySize = offset;
                playListEntryIdsProperty.arraySize = offset;
                playListEntryStartsProperty.arraySize = offset;
                playListEntryEndsProperty.arraySize = offset;
                playListEntryVolumesProperty.arraySize = offset;
                playListEntryFlipsProperty.arraySize = offset;
                ...
                for (int i = 0; i < offset; i++) {
                    ...
                    playListEntryTitleSpellsProperty.GetArrayElementAtIndex(i).stringValue = entry.titleSpell;
                    playListEntryIdsProperty.GetArrayElementAtIndex(i).intValue = entry.id;
                    playListEntryStartsProperty.GetArrayElementAtIndex(i).intValue = entry.start;
                    playListEntryEndsProperty.GetArrayElementAtIndex(i).intValue = entry.end;
                    playListEntryVolumesProperty.GetArrayElementAtIndex(i).doubleValue = entry.volume;
                    playListEntryFlipsProperty.GetArrayElementAtIndex(i).boolValue = entry.flip;
...
#region Playlist Exporters
        ...
        void WriteEntry(JsonWriter jsonWriter, PlayListEntry entry) {
            ...
            jsonWriter.Write(entry.id);
            jsonWriter.WritePropertyName("title");
            jsonWriter.Write(entry.title);
            jsonWriter.WritePropertyName("titleSpell");
            jsonWriter.Write(entry.titleSpell);
            ...
            jsonWriter.WritePropertyName("start");
            jsonWriter.Write(entry.start);
            jsonWriter.WritePropertyName("end");
            jsonWriter.Write(entry.end);
            jsonWriter.WritePropertyName("flip");
            jsonWriter.Write(entry.flip);
            jsonWriter.WritePropertyName("volume");
            jsonWriter.Write(entry.volume);
            jsonWriter.WriteObjectEnd();
        }
        ...
        void LoadEntries(JsonData entries) {
            ...
            foreach (JsonData entry in entries) {
                try {
                    ...
                    var titleSpell = entry["titleSpell"].ToString();
                    ...
                    var end = (int)entry["end"];
                    var flip = (bool)entry["flip"];
                    var volume = (double)entry["volume"];
