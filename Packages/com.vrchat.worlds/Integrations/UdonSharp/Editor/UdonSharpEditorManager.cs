...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void PrepareUdonSharpBehavioursForPlay(IEnumerable<UdonBehaviour> rootBehaviours, bool stripBehavioursForBuild)
        {
            ...
            if (stripBehavioursForBuild)
            {
                ...
                foreach (UdonBehaviour rootBehaviour in rootBehaviours)
                {
                    RemoveUdonBehaviorProxy(rootBehaviour);
                }
            }
        ...
        private static Dictionary<Type, List<Type>> _RequireFieldLookup;

        private static void CalculateLookup()
        {
            _RequireFieldLookup = new Dictionary<Type, List<Type>>();
            var subclasses =
        ...
        private static void RemoveUdonBehaviorProxy(UdonBehaviour rootBehaviour)
        {
            if (UdonSharpEditorUtility.IsUdonSharpBehaviour(rootBehaviour))
            {
                UdonSharpBehaviour proxy = UdonSharpEditorUtility.GetProxyBehaviour(rootBehaviour);
                if(proxy != null)
                    RemoveUdonSharpBehavior(proxy);
            }
        }

        private static void RemoveUdonSharpBehavior(UdonSharpBehaviour proxy)
        {
            if (_RequireFieldLookup.ContainsKey(proxy.GetType()))
            {
                foreach (Type type in _RequireFieldLookup[proxy.GetType()])
                {
                    UdonSharpBehaviour behaviour = proxy.GetComponent(type) as UdonSharpBehaviour;
                    if (behaviour != null)
                    {
                        RemoveUdonSharpBehavior(behaviour);
                    }
                }
            }
